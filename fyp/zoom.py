...
class ZoomAPI:
    ...
    def __init__(self, fyp, std_id, time):
        self.id = fyp
        self.std_id = std_id
        self.time = time
        self.response = None
    ...
    def create_meeting(self):
        self.body['topic'] = self.get_fyp_name()
        self.body['agenda'] = self.get_fyp_description()
    ...
    def fyp_query(self):
        query = FYP.objects.get(id=self.id)
        if query:
            return query
        else:
            self.reject_request()
